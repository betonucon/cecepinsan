...
<div id="content" class="content">
			...
			<div class="row">
				<!-- begin col-3 -->
                @foreach(get_pelayanan() as $pl)
				<div class="col-xl-{{count_pelayanan()}} col-md-6">
					<div class="widget widget-stats bg-{{$pl->color}}">
						<div class="stats-icon"><i class="fa fa-desktop"></i></div>
						<div class="stats-info">
							<h4>{{$pl->pelayanan}}</h4>
							<p>{{countpelayanan($pl->tipe)}}</p>	
						</div>
							...
							<table class="table table-valign-middle table-panel mb-0">
								...
								<tbody>
                                    @foreach(get_pelayanan() as $pl)
                                        <tr>
                                            <td><b>{{$pl->tipe}}</b></td>
                                            <td>{{totalpelayanan(0)}}</td>
                                            <td>{{totalpelayanan(1)}}</td>
                                            <td>{{totalpelayanan(2)}}</td>
                                        </tr>
									@endforeach
								</tbody>
							</table>
